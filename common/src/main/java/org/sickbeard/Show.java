...
import org.sickbeard.model.ShowJson;
import org.sickbeard.model.ShowJson.QualityDetailsJson;
...
public class Show {
	...
	public enum Quality {
		...
		public static Quality fromJson(String quality )
		{
		...
		public static Quality fromOrdinal(int index)
		{
			return Quality.values()[index];
		}
		
		public static EnumSet<Quality> fromBooleans(boolean[] values )
		{
			Quality[] enums = Quality.values();
			if ( values.length != enums.length )
				throw new RuntimeException("Quality.fromBoolens: Array Mismatch values must be length \"" + enums.length + "\"");
			EnumSet<Quality> ret = EnumSet.noneOf(Quality.class);
			for ( int i=0; i < enums.length; i++ ) {
				if ( values[i] )
					ret.add(enums[i]);
			}
